...
function createRL() {
  rl = readline.createInterface({
    ...
    completer: (line) => {
      ...
      if (parts[0] === "/model") {
        const hits = availableModels.filter(m => m.startsWith(parts[1] || ""));
...
function ask() {
  rl.question(`${colorRed}You${reset} > `, async (input) => {
    ...
    if (trimmed === "/help") {
      ...
      console.log(`  /model [id]  - Show or switch model (Tab to autocomplete)`);
    ...
    if (trimmed.startsWith("/model")) {
      ...
      if (!newModel) {
        const selected = await promptSelect("Select a model:", availableModels, currentModel);
        if (selected) {
          currentModel = selected;
          console.log(`${colorBlue}Agent >${reset} Switched to: ${colorYellow}${currentModel}${reset}`);
        } else {
      ...
      if (!availableModels.includes(newModel)) {
        console.log(`${colorRed}Error:${reset} Unknown model "${colorYellow}${newModel}${reset}"`);
    ...
    const res = await runAgent(trimmed, conversation, { model: currentModel });
